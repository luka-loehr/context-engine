    ...
    if (isSubAgentTool(toolName)) {
      ...
      if (callId === Math.min(...Array.from(activeSubAgentCalls.keys()))) {
        ...
        if (allCalls.length > 1) {
          ...
          const result = {
            success: true,
            message: `${fileNames} Created Successfully\n\n- ✅ New files generated based on codebase analysis\n- 📄 Review and customize as needed for your specific project\n- 📝 Generated by context-engine`,
            stopLoop: false // Let AI provide brief feedback
          };
          ...
          // Return result with flags to skip AI response and exit loop
          const result = {
            success: true,
            message: '', // Empty - we already printed the summary
            skipAIResponse: true, // Skip AI summary generation
            stopLoop: true // Exit the chat loop immediately
          };
